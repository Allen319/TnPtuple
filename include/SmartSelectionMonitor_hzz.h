...
struct evt : base_evt{
  ...
  void Fill_evt_withPdgId(TString s_jetCat_, TString s_lepCat_, TLorentzVector boson_, TLorentzVector METVector_, std::vector<TLorentzVectorWithIndex> selJets_, double EvtRunNum_, double EvtVtxCnt_, double EvtFastJetRho_, double METsig_, std::vector<TLorentzVectorWithPdgId> selLeptons_){
    Fill_baseEvt(s_jetCat_, s_lepCat_, boson_, METVector_, selJets_, EvtRunNum_, EvtVtxCnt_, EvtFastJetRho_, METsig_);
    if(selLeptons_.size() > 1){
    lep1pT = selLeptons_[0].Pt();
    lep2pT = selLeptons_[1].Pt();
